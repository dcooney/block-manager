...
function Nav({ blocks }) {	
	...
	const search = () => {
		...
		let blocks = document.querySelectorAll('.gbm-blocks .gbm-block-list button');
				...
				<div id="gbm-sticky">
					<div className="gbm-nav-wrap">
						...
						{blocks && blocks.length &&
							blocks.map((category) => (
								<button key={ category.info.slug } type="button" data-to={'block-'+ category.info.slug } onClick={ moveTo }>{ category.info.title }</button>								
							)
						)}
					</div>
					<div className="gbm-search">
						<label className="offscreen" htmlFor="gbm-search">{ gbm_localize.search_label }</label>
